...
/*   Updated: 2026/03/11 20:48:40 by isastre-         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	create_sphere(t_miniRT *rt, char **params, bool *err)
{
	...
	sp->diameter = atod_err(params[2], err,
			DECIMAL_MIN_POSITIVE_ACCEPTED, DECIMAL_MAX_ACCEPTED);
...
void	create_cylinder(t_miniRT *rt, char **params, bool *err)
{
	...
	cy->diameter = atod_err(params[3], err,
			DECIMAL_MIN_POSITIVE_ACCEPTED, DECIMAL_MAX_ACCEPTED);
	cy->height = atod_err(params[4], err,
			DECIMAL_MIN_POSITIVE_ACCEPTED, DECIMAL_MAX_ACCEPTED);
